...
# IAM Role for dummy-ml-ops
resource "aws_iam_role" "dummy_mlops" {
  name = "dummy-mlops-role"
...
module "s3_data_bucket_to_prefect" {
  ...
  topic_name  = "dummy-data-event-topic"

  webhook_name = "dummy-model-training"
  webhook_template = {
...
module "s3_model_bucket_to_prefect" {
  ...
  topic_name  = "dummy-model-event-topic"

  webhook_name = "dummy-model-inference"
  webhook_template = {
    event = "webhook.called",
    resource = {
      "prefect.resource.id" = "s3.bucket.{{ body.detail.bucket.name }}",
...
resource "prefect_automation" "dummy_train_model" {
...
resource "prefect_automation" "dummy_run_inference" {
  name        = "dummy-run-inference"
  enabled     = true
